...
class Unet_contrastive(nn.Module):
    ...
    def __init__(self, inshape, nb_features=None, nb_levels=None, feat_mult=1):
        ...
        # configure encoder (down-sampling path)
        prev_nf = 1 #default=2，卷积层通道数，此处设置为1是为了提取x,y特征，后面会拼接成2
        self.downarm = nn.ModuleList()
        for nf in self.enc_nf:
    ...
    def forward(self, x, y):
        #moving_img, fixed_img = torch.chunk(x, 2, dim=1)  # 将输入 x 分成两部分，分别是 moving_img 和 fixed_img
        ...
        #y_features = y_enc[-1]
        f_x = self.avgpool(x_features)
        #print("f_x: ",f_x.shape)
        f_x = f_x.squeeze()
        f_x = self.linear_x(f_x)
        f_x = f_x / f_x.norm(dim=-1, keepdim=True)
        f_y = self.avgpool(y_features)
        f_y = f_y.squeeze()
        f_y = self.linear_y(f_y)
        f_y = f_y / f_y.norm(dim=-1, keepdim=True)
        
        # 拼接 x_enc 和 y_enc 的所有内容
        combined_encodings = []
        for x_activation, y_activation in zip(x_enc, y_enc):
            combined_encoding = torch.cat([x_activation, y_activation], dim=1)
            combined_encodings.append(combined_encoding)
            
        # conv, upsample, concatenate series
        x = combined_encodings.pop()
        for i,layer in enumerate(self.uparm):
            x = layer(x)
            x = self.upsample(x)
            
            x = torch.cat([x, combined_encodings.pop()], dim=1) 
...
class VxmDense_contrastive(nn.Module):
    """
    VoxelMorph network for (unsupervised) nonlinear registration between two images.
    """
    def __init__(self,
        ...
        # configure core unet model
        self.unet_model = Unet_contrastive(
            inshape,
            nb_features=nb_unet_features,
            nb_levels=nb_unet_levels,
            feat_mult=unet_feat_mult
...
class VxmDensex2_contrastive(nn.Module):
    """
    VoxelMorph network for (unsupervised) nonlinear registration between two images.
    """
    def __init__(self,
