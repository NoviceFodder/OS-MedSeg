...
class DiceLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        y_pred = nn.functional.one_hot(torch.round(y_pred).long(), num_classes=self.num_class)
        y_pred = torch.squeeze(y_pred, 1)
        y_pred = y_pred.permute(0, 4, 1, 2, 3).contiguous()
        y_true = nn.functional.one_hot(torch.round(y_true).long(), num_classes=self.num_class)
...
class contrastive_loss(nn.Module):
    ...
    def forward(self, emb_i, emb_j):# emb_i, emb_j 是来自同一图像的两种不同的预处理方法得到
        ...
        #print("z_i: ",z_i.shape,"z_j: ",z_j.shape)
...
def cos_hint_loss(teacher_features, student_features):
    """
    计算 hint loss，使用余弦相似度损失

    参数：
    - teacher_features: 老师网络的特征表示，是一个包含多个层特征的列表
    - student_features: 学生网络的特征表示，是一个包含多个层特征的列表

    返回：
    - hint_loss: 计算得到的 hint loss
    """
    # 初始化 hint loss
    hint_loss = 0.0
    ...
    for i in range(len(teacher_features)):
        # 获取老师网络在输入上的第 i 层特征表示
        phi_C_i = teacher_features[i].view(-1)

        # 获取学生网络在映射后在输入上的第 i 层特征表示
        phi_M_i = student_features[i].view(-1)
        #print("phi_C_i: ",phi_C_i.shape)
        #print("phi_M_i: ",phi_M_i.shape)
        # 计算余弦相似度并转换为余弦相似度损失，然后累加到总的 hint loss 中
        cosine_similarity = F.cosine_similarity(phi_C_i, phi_M_i,dim=0)
        #print("cosine_similarity: ",cosine_similarity.shape)
        #print(cosine_similarity)
        hint_loss += 1 - cosine_similarity  # 将余弦相似度转换为余弦相似度损失
    #print("hint loss: ",hint_loss.shape)
    return hint_loss
